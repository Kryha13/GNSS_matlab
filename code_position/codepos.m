...
prn = [  25;  17; 15; 2; 32; 19; 14; 12; 24; 6]; % trzecia epoka trzeci dzieñ 

efemerydy = rinexe('brdc2830.18n', 'brdc2830.nav');  %zmienic nazwe pliku
...
%trzecia epoka z plikuu joz283 - œroda
...
% w zmiennej dtsat znajduje sie poprawka do zeg. satelity
       [Xs, dtsat]   = wspsat(prn(ii), efemerydy, tobs+dtrec-tau);
       [Xs1, dtsat1]   = wspsat(prn(ii), efemerydy, tobs+dtrec-tau +0.1);
...
% azymut, wysokosc nad horyzontem oraz odleglosc do satelity
       ...
       if wys(ii)>=15
           isat=isat+1;
           PACT(isat)=P3(ii);
% !!!! Poprawki do pseudoodlegÅ‚oÅ›ci, ktÃ³re naleÅ¼y uwzglÄ™dniÄ‡:
           drel(ii)= -(2*Xsat'*V)/vc;
...
% plot(prn, azymut, 'o')
% xlabel('Numer satelity GPS')
% ylabel('Wartoœæ azymutu [°]')
% title('Azymuty do satelitów')
% grid on
% 
% figure
% plot(prn, wys, 'o')
% xlabel('Numer satelity GPS')
% ylabel('Wartoœæ elewacji [°]')
% grid on
% title('Elewacja satelitów')